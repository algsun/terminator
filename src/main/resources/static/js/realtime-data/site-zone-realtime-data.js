...
var REAL_TIME = {
    ...
    // 固定列
    $device_realTime_dataTable_body: ".DTFC_LeftBodyLiner",
    // 滚动列
    ...
    initRealTimeData: function (data) {
        var REAL_TIME = this;
        if (data.length < 1) {
            REAL_TIME.$realTimeDiv.append("<h4>" + message.noDevice + "</h4>");
            return;
        }
        ...
        $.each(data, function (i, dataRow) {
            deviceIds.push(dataRow.locationId);
            $realTimeDataTable.children("tbody").children("#" + dataRow.locationId).children().each(function (index, td) {
                var $td = $(td);
                var dataType = $td.attr("data-type");
                if ($td.attr("data-dead") === "1") {
                    REALTIME_DATA_UTIL.baseAttrView($td, dataType, dataRow, serverTime);
                } else {
                    if (dataRow.locationSensorInfoMap != null && dataRow.sensorinfoMap == null) {
                        if (dataRow.locationSensorInfoMap[dataType]) {
                            $td.removeAttr('style');
                            if (dataRow.locationSensorInfoMap[dataType].valueState == 3) {
                                classValue = '';
                            } else if (dataRow.locationSensorInfoMap[dataType].valueState == 1) {
                                classValue = 'background-color:#faf2cc';
                                $td.attr("style", classValue)
                            } else if (dataRow.locationSensorInfoMap[dataType].valueState == 2) {
                                classValue = 'background-color:#c4e3f3';
                                $td.attr("style", classValue)
                            }
                            $td.attr("title", dataRow.locationSensorInfoMap[dataType].cnName);
                            if(dataRow.locationSensorInfoMap[dataType].state == 0 ) {
                                $td.text("--");
                            } else {
                                /*if(dataRow.locationSensorInfoMap[dataType].showType==3) {
                                    $td.text(dataRow.locationSensorInfoMap[dataType].sensorPhysicalValue==0 ? '关' : '开');
                                }else {*/
                                    $td.text(dataRow.locationSensorInfoMap[dataType].sensorPhysicalValue);
                                //}
                            }
                        }
                    } else if (dataRow.locationSensorInfoMap == null && dataRow.sensorinfoMap != null) {
    ...
    renderRealTimeDataView: function (data) {
        ...
        $.each(data, function (i, dataRow) {
            // 初始化 设备实时数据
            $(REAL_TIME.$device_realTime_dataTable_body).children("table").children("tbody").children("#" + dataRow.locationId).children().each(function (index, td) {
                REALTIME_DATA_UTIL.baseAttrView($(td), $(td).attr("data-type"), dataRow, serverTime);
            });
            $(REAL_TIME.$realTime_dataTable_body).children("table").children("tbody").children("#" + dataRow.locationId).children().each(function (index, td) {
                var $td = $(td);
                var dataType = $td.attr("data-type");
                if ($td.attr("data-dead") === "1") {
                    REALTIME_DATA_UTIL.baseAttrView($td, dataType, dataRow, serverTime);
...
/*
                    dataRow.locationSensorInfoMap[dataType].state == 0 ? $td.text("--") : $td.text(dataRow.locationSensorInfoMap[dataType].sensorPhysicalValue);
*/
                }
            });
            ...
            for (var i = 0; i < REAL_TIME.preData.length; i++) {
                ...
                if (dataRow.locationId == REAL_TIME.preData[i].locationId) {
                    var $appendTr0 = $(REAL_TIME.$device_realTime_dataTable_body).children("table").find("." + dataRow.locationId);
                    /*var $appendTr1 = $(REAL_TIME.$realTime_dataTable_body).children("table").find("#" + dataRow.locationId);*/
                    var $tr3 = $("#real_time_data_table_mould").find("#" + dataRow.locationId);
                    if (dataRow.stamp != REAL_TIME.preData[i].stamp) {
                        ...
                        $tr3.css("backtround-color", "transparent");
                        // lightgreen => #90ee90
                        ...
                        $tr3.animate({backgroundColor: "#90ee90"}, 1000)
                            .animate({backgroundColor: "transparent"}, 3000, function () {
